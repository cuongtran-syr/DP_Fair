file_path = '/home/cutran/Documents/privacy_with_fairness/res/'
from FairModel import *
import time
# SETTINGS for INCOME DATA
def get_params(pd00, feats, options):

  x_train, y_train, x_control_train, x_test, y_test, x_control_test, train_df, test_df, X_train, X_val, Z_train, Z_val, Y_train, Y_val = get_data_v2(
        pd00, feats, seed= 0, bs=200)

  gen_params = copy.deepcopy(options)
  gen_params['X_val'] = X_val
  gen_params['y_val'] = Y_val

  gen_params['z_val'] = Z_val
  gen_params['X_train'] = X_train
  gen_params['y_train'] = Y_train
  gen_params['z_train'] = Z_train
  gen_params['delta'] = 1e-5
  gen_params['bs'] = 256
  gen_params['task'] = 'clf'

  return gen_params


def rand_response(z, num_z,eps):
  main_prob = np.exp(eps)/(num_z -1 + np.exp(eps))
  aux_prob = (1 - main_prob)/float(num_z -1)
  pr = [aux_prob]  * num_z
  pr[int(z)] = main_prob
  return np.random.choice(list(range(num_z)), p= pr)


def test2(data):
    print('Working with data ', data)

    document = "Run Our Private Fair Models w.r.t second DP Definition"

    num_epochs = 200
    res = {}
    num_seed = 10
    eps_list = np.linspace(0.2, 2.0, 20).tolist()

    res[data] = {'ACC': {}, 'PR': {}, 'Both':{}}
    res['document'] = document
    for key_ in res[data].keys():
        res[data][key_] = {}
        for choice in ['1st', '2nd']:
            res[data][key_][choice] = {}

            for eps in eps_list:
                 res[data][key_][choice][eps] = []

    if data == 'bank':
        pd00, feats = load_bank_dataset()
        num_z = 2
    elif data == 'income':
        pd00, feats = load_adult_dataset()
        num_z = 2
    elif data == 'biased_income':
        pd00 = pd.read_csv('/content/drive/My Drive/research/privacy_with_fairness/temp_data/reduced_bias_income.csv')
        feats = [col for col in pd00.columns.tolist() if col not in ['z', 'label', 'intercept']]
    elif data =='default':
        pd00, feats = load_default_dataset()
        num_z = 2
    elif data =='compas':
        pd00, feats = load_compas_data()
        num_z = 2
    elif data == 'multi_group_income':
        pd00, feats = load_multi_group_adult_dataset()
        num_z = 5


    for fair_choice in ['ACC', 'PR', 'Both']:


        for choice in ['1st', '2nd']:
            for eps in eps_list:
                for seed in range(num_seed):
                    if data == 'income' and choice =='1st' and fair_choice =='ACC':
                        mult_lr = [0.4] *4
                    elif  data == 'bank' and choice =='1st' and fair_choice =='ACC':
                        mult_lr = [0.1]*4
                    else:
                        mult_lr = [1e-3]*4

                    options = {'dp': False, 'clip_norm': False, 'C': 1e20, 'sigma': 1e-50, 'epochs': num_epochs,
                               'lambda_': [0.0] * 4,
                               'mult_lr': mult_lr , \
                               'clf': True, 'noise_rng': None, 'device': 'cpu', 'C_2': 1e10, 'fair_choice': fair_choice,
                               'delta': 1e-5, \
                               'sigma_2': 1e-50, 'second_order': None}

                    options['model_params'] = {'i_dim': len(feats) + 1,
                                               'h_dim': [int(2 * len(feats) / 3), int(len(feats) / 2)],
                                               'o_dim': 1, 'n_layers': 2}
                    if choice == '1st':
                        options['second_order'] = False
                    else:
                        options['second_order'] = True

                    gen_params = get_params(pd00, feats, options)
                    pd01 = pd00.copy(deep=True)
                    pd01['z'] = pd01['z'].apply(lambda x: rand_response(x, num_z, eps))
                    gen_params1 = get_params(pd01, feats, options)
                    gen_params1['X_val'] = gen_params['X_val']
                    gen_params1['y_val'] = gen_params['y_val']
                    gen_params1['z_val'] = gen_params['z_val']
                    fair_model = FairModel(gen_params1)
                    fair_model.fit(options)
                    if (data =='income' and fair_model.logs['all_acc'][-1]<0.75) or (data =='compas' and fair_model.logs['all_acc'][-1]<0.52):
                        print('carefull, acc is so low ')

                    res[data][fair_choice][choice][eps].append(copy.deepcopy(fair_model.logs))

    file_name = file_path + 'v5_ICDM_{}_fair_with_input_perturbation.pkl'.format(data)

    file_handle = open(file_name, 'wb')
    pickle.dump(res, file_handle)


if __name__ == "__main__":

    starttime = time.time()

    data = 'income'
    test2(data)


    print('That took {} seconds for {} dataset'.format(time.time() - starttime, data ))