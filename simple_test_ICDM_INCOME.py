...
def test(data, C_choice, sigma_choice, seed ):
    print('Working with data ', data)

    document = "Run Our Private Fair Models w.r.t second DP Definition"
    num_epochs = 50
    C_choice = float(C_choice)
    sigma_choice = float(sigma_choice)
    res = {}
    res[data] = {'ACC': {}, 'PR': {}, 'Both':{}}
    res['document'] = document
    for key_ in res[data].keys():
        for sigma in [sigma_choice]:
            res[data][key_][sigma] = {}
            for C in [C_choice]:
                res[data][key_][sigma][C] = None
    ...
    for fair_choice in ['ACC', 'PR', 'Both']:
        for sigma in [sigma_choice]:
            ...
            for C in [C_choice]:
                ...
                res[data][fair_choice][sigma][C] =  copy.deepcopy(fair_model.logs)

    file_name = file_path + 'income/v5_ICDM_{}_private_fair_C_{}_sigma_{}_seed_{}_v2.pkl'.format(data,C_choice, sigma_choice, seed)
...
def main():
   ...
   test('income', args.C,args.sigma, args.seed)
